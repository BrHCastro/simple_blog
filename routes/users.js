...
const User = mongoose.model('user')
    ...
    router.post('/register', (req, res) => {
        ...
        if (err.length > 0) {
            ...
            .then((user) => {
                if (user) {
                    req.flash('error_msg', `Já existe um usuário com o e-mail "${req.body.email}"`)
                    ...
                    .then(()=>{
                        req.flash('success_msg', `Boa, ${req.body.name}! Seu cadastro foi realizado com sucesso! Clique <a href="#">aqui</a> e faça o login.`)
                        res.redirect('/user/register')
                    }).catch((err)=>{
